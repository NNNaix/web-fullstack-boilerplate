...
function getCssLoaders(importLoaders: number, inDev: boolean) {
    return [
...
export function generateCommonConfig({
    runtime: { appRootPath, appEnv },
    dev,
    staticRootPath,
}: ServerSectionSetting) {
    const inDev = appEnv === AppEnvironment.DEVELOPMENT;
    const commonConfig: Configuration = {
        cache: true,
        context: appRootPath,
        entry: ['react-hot-loader/patch', resolve(appRootPath, './client/app/index.tsx')],
        output: {
            publicPath: '',
            path: resolve(appRootPath, staticRootPath),
            filename: 'js/[name]-[hash].bundle.js',
            // chunkFilename: 'js/[name].bundle.js',
            hashSalt: 'app-webfront',
        },
        resolve: {
        ...
        plugins: [
            ...
            new CopyPlugin([
                {
                    context: resolve(appRootPath, './client'),
                    from: '*',
                    to: resolve(appRootPath, './dist'),
                    toType: 'dir',
                },
            ]),
        ],
        module: {
            rules: [
                ...
                {
                    test: /\.css$/,
                    use: getCssLoaders(0, inDev),
                },
                {
                    test: /\.less$/,
                    use: [
                        ...getCssLoaders(2, inDev),
                        {
                            loader: 'less-loader',
                            options: {
                                sourceMap: true,
    ...
    if (inDev) {
        // 开发环境下注入热更新补丁
        const { hmrPath, hmrReload, hmrOverlay } = dev as ServerDevSectionSetting;
        (commonConfig.entry as string[]).unshift(
            `webpack-hot-middleware/client?path=${hmrPath}&reload=${hmrReload}&overlay=${hmrOverlay}`,
        );
    }
    return commonConfig;
}
